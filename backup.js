...
exports.copyConcessionDetailsToHistory = functions.firestore
    ...
    .onWrite((change, context) => {
      ...
      if (!newData || newData.status != "serviced") {
        console.log("Skipping update to history.");
        return null;
      }
...
exports.sendNotificationOnAnnouncement = functions.firestore
    .document("ImportantNotice/Content")
    .onUpdate(async (change, context) => {
      ...
      if (afterContent.length > beforeContent.length) {
        const newEntries = afterContent.slice(beforeContent.length);

        for (const entry of newEntries) {
          let gradYear = entry.gradYear;
          ...
          if (gradYear === "All") {
            // Query for all students which is same as above
          } else {
            // Filter acc to the data available

            if (branch === "All") {
              branch = div = batch = null;
            } else if (div === "All") {
              div = batch = null;
            } else if (batch === "All") {
              batch = null;
          ...
          try {
              ...
              if (tokens.length > 0) {
                const message = {
                  notification: {
                    ...
                    body: entry.title,
                    image: "https://firebasestorage.googleapis.com/v0/b/tsec-app.appspot.com/o/Announcement%2Fannouncement.png?alt=media&token=1241a337-9273-472e-b2d4-a1b9b5187808",
                  },
                  data: {
                    imageUrl: "https://firebasestorage.googleapis.com/v0/b/tsec-app.appspot.com/o/Announcement%2Fannouncement.png?alt=media&token=1241a337-9273-472e-b2d4-a1b9b5187808",
                  },
                  tokens: tokens,
                };
            ...
            console.error("Error querying or sending message:", error);
          }
        }
      }
