  ...
  const fetchAllEnquiries = async (travelLane) => {
    try {
      ...
      const servicedRequestsSnapshot = await getDocs(
        query(concessionRequestRef, where("status", "==", "serviced"), limit(100))
      );
      ...
      if (fetchedEnquiries.length === 0) {
        toast.notify("No serviced requests found for this lane and status.", {
          type: "info",
        });
        return;
      }
      ...
      const lastName = await getPassNumFromConcessionRequest(
        fetchedPassNum[fetchedEnquiries.length - 1]
      );
    ...
    try {
      const enquiriesWithPassNum = await Promise.all(
        jsonData.map(async (enquiry) => {
          ...
          const name = [enquiry.firstName, enquiry.middleName, enquiry.lastName]
            .filter(Boolean)
            .join(" ");
          const dob = convertDate(enquiry.dob.seconds, enquiry.dob.nanoseconds);
          const lastPassIssued = convertDate(
            enquiry.lastPassIssued.seconds,
            enquiry.lastPassIssued.nanoseconds
          );
